...
img = Image.open(r'_inven.jpeg')
img= ImageTk.PhotoImage(img.resize((1050, 520), Image.ANTIALIAS))
...
def Home():
    ...
    lbl_display.pack()
    b = Button(Home, text="View Inventory", command=ShowView)
    b.pack()
    b1 = Button(Home, text="Add new items", command=ShowAddNew)
    b1.pack()
    b2 = Button(Home, text="Log out", command=Logout)
    b2.pack(side=BOTTOM)
    menubar = Menu(Home)
...
lbl_display = Label(Title, text=" Inventory System", font=('arial', 45))
lbl_display.pack()
b = Button(root, text="Log in", command=ShowLoginForm)
